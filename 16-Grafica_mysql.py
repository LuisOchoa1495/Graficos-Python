...
my_data2 = pd.read_sql("SELECT c.categoria,sum(p.cantidad) as total FROM productos as p INNER JOIN categorias as c ON c.idcategoria=p.categoria GROUP BY c.categoria",conexion)
labels=my_data2['categoria']
print(labels)
my_data2.plot(y="total", kind="pie", autopct='%1.01f%%',labels=labels,legend=False)
plt.title("Total de productos por categorias")
plt.show()
